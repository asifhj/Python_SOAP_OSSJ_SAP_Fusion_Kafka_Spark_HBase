...
class Utils:
    ...
    def validate_sr_details(self, document, row):
        ...
        if r.status_code==200:
            ...
            endDate = ""
            try:
                endDate = ("" if response['entitlement']['endDate'] is None else str(response['entitlement']['endDate']).strip())
                if not ( endDate == "00000000" or endDate == "" ):
            ...
            if not (str(document['ouatgeCauseDescription']).strip()) == ("" if response['outage']['outageCause'] is None else str(response['outage']['outageCause']).strip()):
                tmp = [str(document['ouatgeCauseDescription']).strip(), str(response['outage']['outageCause']).strip()]
                tmp.append("Incorrect value for 'ouatgeCauseDescription/outageCause'!")
                table.append(tmp)
                status = 1
            ...
            startDate = ""
            try:
                startDate = ("" if response['entitlement']['startDate'] is None else str(response['entitlement']['startDate']).strip())
                if not (startDate == "" or startDate == "000000"):
            ...
            if not (str(document['startDate']).strip())== startDate:
    ...
    def validate_sr_attachments(self, document, row):
        print "API URL: "+self.url+"case-manager/cases/"+str(document['caseId'])
        r = requests.get(self.url+"case-manager/cases/"+str(document['caseId']))
        print "CaseID: "+str(document['caseId'])
        print "Response: "+str(r.status_code)
